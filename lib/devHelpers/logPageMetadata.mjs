...
function main() {
  const staticPages = getStaticPageEntries()
    .map(getStaticPageMetadata)
    .filter(Boolean);
  const contentPages = getContentPageMetadata();
  const paginatedPages = getBlogPaginationMetadata();
  const tagPages = getTagPageMetadata();

  const pages = [...staticPages, ...contentPages, ...paginatedPages, ...tagPages].sort((a, b) =>
    a.route.localeCompare(b.route, 'fi')
  );
