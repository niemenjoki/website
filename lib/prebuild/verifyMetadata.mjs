...
import { createPageMetadata } from '../metadata/createPageMetadata.js';
import {
  createBlogPageMetadata,
  createBlogTagPageMetadata,
  createContentPageMetadata,
  notFoundPageMetadata,
} from '../metadata/routeMetadata.js';
...
function validateStaticPages() {
  ...
  for (const page of staticSitePages) {
    ...
    const metadata = createPageMetadata(page.metadata);

    if (metadata.title && metadata.description) {
      validateTitleAndDescription(metadata.title, metadata.description, page.canonicalUrl);
    }
  }

  validateTitleAndDescription(
    notFoundPageMetadata.title,
    notFoundPageMetadata.description,
    '/_not-found'
  );
}
...
function validateContentPages() {
  ...
  for (const post of posts) {
    ...
    const routeMetadata = createContentPageMetadata(post.slug);
...
function validatePaginatedPageLengths() {
  ...
  for (let pageIndex = 1; pageIndex <= blogNumPages; pageIndex++) {
    const pageIndexString = String(pageIndex);
    const pageData = getBlogPageData(pageIndexString);
    const metadata = createBlogPageMetadata(pageIndexString);

    validateTitleAndDescription(metadata.title, metadata.description, pageData.pagePath);
  }
  ...
  for (const rawTag of getAllPostTags()) {
    ...
    for (let pageIndex = 1; pageIndex <= numPages; pageIndex++) {
      const pageIndexString = String(pageIndex);
      const pageData = getBlogTagPageData({ tag, pageIndex: pageIndexString });
      const metadata = createBlogTagPageMetadata({ tag, pageIndex: pageIndexString });
