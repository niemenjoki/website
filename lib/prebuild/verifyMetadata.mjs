...
function validateStaticStructuredDataFile(filePath, route) {
  ...
  const primaryType = jsonLd.primaryNode['@type'];

  if (primaryType === 'ProfilePage' && !jsonLd.primaryNode.mainEntity) {
    fail(`[${label}] ProfilePage is missing mainEntity`);
  }

  if (primaryType === 'CollectionPage' && !findGraphNode(jsonLd.graph, 'ItemList')) {
    fail(`[${label}] CollectionPage is missing an ItemList node`);
  }

  if (jsonLd.primaryNode.breadcrumb && !findGraphNode(jsonLd.graph, 'BreadcrumbList')) {
    fail(`[${label}] JSON-LD references breadcrumb but is missing a BreadcrumbList node`);
  }
...
function validateDynamicStructuredDataTemplate(filePath, route) {
  ...
  const jsonLd = validateJsonLdStructure(data, label, {
    expectedUrl: absoluteUrl(route),
    allowedPrimaryTypes: ['WebPage', 'CollectionPage'],
    allowedPlaceholders,
    requireName: true,
  });
  ...
  if (jsonLd.primaryNode.breadcrumb && !findGraphNode(jsonLd.graph, 'BreadcrumbList')) {
    fail(
      `[${label}] JSON-LD template references breadcrumb but is missing a BreadcrumbList node`
    );
  }
}
...
function validatePageJsonLd(entry, pageSource) {
  ...
  if (importMatch) {
    if (!/type=["']application\/ld\+json["']/.test(pageSource)) {
      fail(
        `[${toProjectPath(entry.filePath)}] structuredData.json is imported but not rendered`
      );
    }
  ...
  if (toProjectPath(entry.filePath) === 'app/blogi/julkaisu/[slug]/page.jsx') {
    if (!/type=["']application\/ld\+json["']/.test(pageSource)) {
      fail(
        `[${toProjectPath(entry.filePath)}] Content page is missing JSON-LD rendering`
      );
    }

    if (!pageSource.includes('structuredData')) {
      fail(
        `[${toProjectPath(entry.filePath)}] Content page does not render content JSON-LD`
      );
    }
  }
}
  ...
  if (fs.existsSync(structuredDataPath)) {
    ...
    if (jsonLd?.primaryNode) {
      ...
      if (description && jsonLdDescription !== description) {
        fail(
          `[${label}] Metadata description does not match structuredData.json description`
        );
      }
    }
  }
...
function validateStaticPageLengths(pageEntries) {
  for (const entry of pageEntries) {
    ...
    if (
      ...
      toProjectPath(metadataSource.filePath) ===
        'app/blogi/sivu/[pageIndex]/generateMetadata.js'
    ) {
